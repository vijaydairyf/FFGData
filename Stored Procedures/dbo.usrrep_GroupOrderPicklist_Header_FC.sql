...
-- =============================================
CREATE PROCEDURE [dbo].[usrrep_GroupOrderPicklist_Header_FC]

--exec [dbo].[usrrep_GroupOrderPicklist_Header_FC] 'FC','2019/01/18','2019/01/18','SAWYERS','','FFGSO361763'
...
SET @TotalKGQTY = (Select SUM(CAST(sl.[Original Ordered Qty_] as decimal(18,2))) as TotalKGQTY
			 ...
			 left join [FM_Innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 

			 	where 
					sh.[Shortcut Dimension 1 Code] COLLATE DATABASE_DEFAULT = @Site -- changed temp KH 
					and sh.[Shipment Method Code] COLLATE DATABASE_DEFAULT = @Shipmethod 
					and po.[name] COLLATE DATABASE_DEFAULT IN ( Select OrderNoParaValues from @OrderNoParamTable) 
...
Case when substring(pol.description6,1,1) = 0 THEN 'All'
	...
	from [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Header]   as sh 
		left join [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Line] as sl with (nolock) on sl.[Document No_] COLLATE DATABASE_DEFAULT = sh.[No_] 
		left join [FM_Innova].[dbo].proc_orders as po with (nolock) on po.[name] COLLATE DATABASE_DEFAULT =  sh.[No_] 
		left join [FM_Innova].[dbo].proc_materials as pm with (nolock) on  sl.No_ COLLATE DATABASE_DEFAULT = pm.extcode 
		left join [FM_Innova].[dbo].proc_orderl as pol with (nolock) on po.[order] = pol.[order] and pol.material = pm.material
		left join [FM_Innova].[dbo].base_companies as bc with (nolock) on bc.company = po.customer

	
	
	where 
	sh.[Shortcut Dimension 1 Code] COLLATE DATABASE_DEFAULT = @Site 
	and sh.[Shipment Method Code] COLLATE DATABASE_DEFAULT = @Shipmethod 
	and 
