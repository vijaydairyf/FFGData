...
-- =============================================
Create PROCEDURE [dbo].[usrrep_GroupOrderPicklist_Header_FI]

--exec [dbo].[usrrep_GroupOrderPicklist_Header] 'FI','07/05/2018', '07/05/2018','SAWYERS','FFGSO306490'
--exec [dbo].[usrrep_GroupOrderPicklist_Header] 'FO','FFGSO253285'
...
SET @TotalKGQTY = (Select SUM(CAST(sl.[Original Ordered Qty_] as decimal(18,2))) as TotalKGQTY
			 ...
			 left join [FI_Innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
...
Case when substring(pol.description6,1,1) = 0 THEN 'All'	-- NONE REMVOED
	 when substring(pol.description6,1,1) = 1 THEN 'Dates'  --- REMOVES  LOTS
	 when substring(pol.description6,1,1) = 2 THEN 'Lots'	--- REMOVE DATES
	 when substring(pol.description6,1,1) = 3 THEN 'None'	-- ALL REMVOED
	 end	as Constraints,
	 pol.description6
	...
	from [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Header] as sh 
		...
		left join [FI_Innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
		left join [FI_Innova].[dbo].proc_materials as pm with (nolock) on  sl.No_ COLLATE DATABASE_DEFAULT = pm.extcode 
		left join [FI_Innova].[dbo].proc_orderl as pol with (nolock) on po.[order] = pol.[order] and pol.material = pm.material
		left join [FI_Innova].[dbo].base_companies as bc with (nolock) on bc.company = po.customer
