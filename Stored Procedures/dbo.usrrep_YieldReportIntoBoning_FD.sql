...
CREATE PROCEDURE [dbo].[usrrep_YieldReportIntoBoning_FD]
...
from	[FD_innova].[dbo].proc_matxacts AS pmx
		LEFT Join [FD_innova].[dbo].proc_materials AS pm with (nolock) on pm.material = pmx.material
		LEFT Join [FD_innova].[dbo].proc_xactpaths AS xp with (nolock) on xp.xactpath = pmx.xactpath
		LEFT JOin [FD_innova].[dbo].proc_lots AS pl with (nolock) on pl.lot = pmx.tolot
...
from	[FD_innova].[dbo].proc_matxacts AS pmx
		...
		LEFT JOin [FD_innova].[dbo].proc_lots AS pl with (nolock) on pl.lot = pmx.tolot
		LEFT Join [FD_innova].[dbo].proc_xactpaths AS xp with (nolock) on xp.xactpath = pmx.xactpath
		LEFT JOIN 	[FD_innova].[dbo].proc_items as it with (nolock) on it.id = pmx.item
		LEFT JOIN [FD_innova].[dbo].proc_prunits AS pr WITH (NOLOCK) ON pr.prunit = ISNULL(it.inventory,'0')
		Inner JOIN [dbo].[Split](@_Lots, ',') As S on pl.code = S.Item
...
from	[FD_innova].[dbo].proc_matxacts AS pmx
		...
		LEFT Join [FD_innova].[dbo].proc_xactpaths AS xp with (nolock) on xp.xactpath = pmx.xactpath
		LEFT JOIN 	[FD_innova].[dbo].proc_items as it with (nolock) on it.id = pmx.item
		LEFT JOIN [FD_innova].[dbo].proc_prunits AS pr WITH (NOLOCK) ON pr.prunit = it.inventory
		Inner JOIN [dbo].[Split](@_Lots, ',') As S on pl.code = S.Item
