...
-- =============================================
Create PROCEDURE [dbo].[usrrep_GroupOrderPicklist_Header_FD]

--exec [dbo].[usrrep_GroupOrderPicklist_Header] 'FD','05/18/18','05/18/18','RTS','FFGSO309717'
...
SET @TotalKGQTY = (Select SUM(CAST(sl.[Original Ordered Qty_] as decimal(18,2))) as TotalKGQTY
			 ...
			 left join [FD_Innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
...
Case when substring(pol.description6,1,1) = 0 THEN 'All'
	...
	from [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Header] as sh 
		...
		left join [FD_Innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
		left join [FD_Innova].[dbo].proc_materials as pm with (nolock) on  sl.No_ COLLATE DATABASE_DEFAULT = pm.extcode 
		left join [FD_Innova].[dbo].proc_orderl as pol with (nolock) on po.[order] = pol.[order] and pol.material = pm.material
		left join [FD_Innova].[dbo].base_companies as bc with (nolock) on bc.company = po.customer
