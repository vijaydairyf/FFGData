...
--exec [dbo].[FM_Innova_ETL_Packs] 

CREATE PROCEDURE [dbo].[FM_Innova_ETL_Packs] 
...
where LRP.TableName = 'tblPack' and [Site]='FC'
...
SELECT	'Foyle Campsie' as [StockSite], pp.number as PackNumber, sites.[name] as OrginalSite, pp.sscc as SSCC, PL.code AS LotCode, PL.[name] as LotName, IsNull(PC.number,'') as PalletNo, IsNull(PP.prday,'1753-01-01') as ProductionDate, 
...
left join [FM_Innova].[dbo].[base_devices] as BD with (nolock) on BD.device = PP.device
left join [FM_Innova].[dbo].[proc_prunits] as PR with (nolock) on  PP.inventory=PR.prunit
left join [FM_Innova].[dbo].[proc_invlocations] as INVLOC with (nolock) on INVLOC.id = pp.invlocation
left join [FM_Innova].[dbo].[proc_materials] as MAT with (nolock) on MAT.material = PP.material
left join [FM_Innova].[dbo].[proc_conditions] as COND with (nolock) on COND.condition = MAT.condition
left join [FM_Innova].[dbo].[proc_conservations] as CONS with (nolock) on CONS.conservation = MAT.conservation
left join [FM_Innova].[dbo].[proc_materialtypes] as MATTYPE with (nolock) on MATTYPE.materialtype = MAT.materialtype
left join [FM_Innova].[dbo].[base_sites] as Sites with (nolock) on  Sites.[site] = pp.[site] -- added by KH -- Changed From Dimension1 to Site by MMG 06/04/2017
left join [FM_Innova].[dbo].[proc_collections] as PC with (nolock) on PC.id = PP.pallet
left join [FM_Innova].[dbo].[base_companies] co WITH (NOLOCK) on PL.processor = co.company
...
using #tmp AS tmp
on pit.sscc = tmp.sscc and pit.[StockSite] = 'Foyle Campsie'
...
From [FFGData].[dbo].[tblLRP] Where TableName='tblPack' and [Site] = 'FC'
