...
--EXEC [APP_Stock_NAV_Interface]
CREATE PROCEDURE [dbo].[APP_Stock_NAV_Interface] 
 ...
 *
  ...
  from [FFGSQLTEST].[PHASE2].[dbo].[VW_NAV_AVAILABLE_STOCK]

  --Where [global dimension 1 code] = 'FT'
...
-- select * from  [FFGSQLTEST].[PHASE2].[dbo].[APP_NAV_TRANSACTIONS_LOGS]

/*
update 
--select * from 
[FFGSQLTEST].[PHASE2].[dbo].[APP_NAV_TRANSACTIONS_LOGS]
set Processed = 0
where cast([Imported to nav] as date) = CAST('2017-01-11' as date)
*/
declare @orderLogCheck int

-------------------------------------------------------------------------------------------------------------
set @orderLogCheck = (select count(*) from [FFGSQLTEST].[PHASE2].[dbo].[Phase2$Order Log] with (nolock) where cast([Date Imported] as date) = cast(getdate() as date) AND Processed = 0 and [Errors Exist] <> 1 and [Message Type] = 'STK'  )
...
--INPUT TRANSACTIONS INTO OUR LOG TABLE - SP Will take from LOG Table to Order Import Table every 2 minutes
--------------------------------------------------------------------------------------------
Insert into [FFGSQLTEST].[PHASE2].[dbo].[APP_NAV_TRANSACTIONS_LOGS] 
