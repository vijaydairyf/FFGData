...
-- =============================================
CREATE PROCEDURE [dbo].[usrrep_GroupOrderPicklist_Header_FO]

--exec [dbo].[usrrep_GroupOrderPicklist_Header_FO] 'FO','11/13/2018', '11/13/2018','SAWYERS','','FFGSO348010'
--exec [dbo].[usrrep_GroupOrderPicklist_Header_FO] 'FO','FFGSO253285'
...
SET @TotalKGQTY = (Select SUM(CAST(sl.[Original Ordered Qty_] as decimal(18,2))) as TotalKGQTY
			 ...
			 left join [FO_Innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
...
Case when substring(pol.description6,1,1) = 0 THEN 'All'
	...
	from [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Header] as sh 
		...
		left join [FO_Innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
		left join [FO_Innova].[dbo].proc_materials as pm with (nolock) on  sl.No_ COLLATE DATABASE_DEFAULT = pm.extcode 
		left join [FO_Innova].[dbo].proc_orderl as pol with (nolock) on po.[order] = pol.[order] and pol.material = pm.material
		left join [FO_Innova].[dbo].base_companies as bc with (nolock) on bc.company = po.customer
