...
-- =============================================
CREATE PROCEDURE [dbo].[usrrep_PalletvsOrderline_Check_FG]
...
left join [FG_innova].[dbo].proc_prunits as inv with (nolock) on inv.prunit = pp.inventory
...
left join [FG_innova].[dbo].proc_orderl as pol with (nolock) on pol.[order] = po.[order]
...
from [FG_innova].[dbo].proc_packs as pp
left join [FG_innova].[dbo].proc_collections pc with (nolock) on pc.id= pp.pallet
left join [FG_innova].[dbo].proc_lots as pl with (nolock) on pl.lot = pp.lot 
left join [FG_innova].[dbo].proc_materials as pm with (nolock) on pm.material = pp.material
