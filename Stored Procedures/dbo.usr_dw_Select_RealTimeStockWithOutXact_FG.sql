...
-- =============================================
-- exec [usr_dw_Select_RealTimeStockWithOutXact_FG]
CREATE PROCEDURE [dbo].[usr_dw_Select_RealTimeStockWithOutXact_FG]
...
select 
	...
	(
		select code 
		from [FG_INNOVA].dbo.vw_matswithNoXML with (nolock) 
...
left join [FG_innova].[dbo].vw_matswithNoXML mat (nolock) on PP.material = mat.material
inner join [FFGBI-SERV].FFG_DW.dbo.stock_packs SP (nolock) on sp.id = pp.id and sp.siteid = 5
...
and	(sp.id = pp.id 
	...
	or 
	sp.InventoryLocationID <> pp.invLocation 
	or 
	sp.Palletid <> pp.Pallet 
	or 
	SP.deviceid <> pp.Device 
	or 
	pp.rtype <> sp.rtype 
	or 
	pp.[weight] <> sp.[weight])
