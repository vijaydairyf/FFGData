...
CREATE PROCEDURE [dbo].[USREP_HR_Dispatch_Summary_Report_FD] 
...
--set @NAVOrder = ('%' + cast(@OrderNo as nvarchar(15))  + '%')
		select 'FD' as site,count(pk.id) as QTY  ,pm.description1 collate SQL_Latin1_General_CP1_CI_AS as [Product Name] , 
		pl.description1 collate SQL_Latin1_General_CP1_CI_AS as [Supplier ID],po.Dispatchtime,
		sh.[Delivery Date], DATENAME(dw,sh.[Delivery Date]) as DOW ,sh.[External Document No_] as PO,'Foyle Donegal IE 292 EC' as Supplier 
	From [FD_INNOVA].dbo.[proc_packs] pk
	left join [FD_INNOVA].[dbo].[vw_OrderswithNoXML] po (nolock) on pk.[order]= po.[order]
	left join [FD_INNOVA].[dbo].[vw_matswithNoXML] pm (nolock) on pk.material = pm.material
	left join [FD_INNOVA].[dbo].[vw_LotswithNoXML] pl (nolock) on pk.lot = pl.lot
	left join [FD_INNOVA].dbo.[proc_collections] pc (nolock) on pk.pallet = pc.id
