...
CREATE PROCEDURE [dbo].[usrrep_YieldReportOutput_FD_HistoricalValue]
...
from [FD_Innova].[dbo].Proc_Lots
Inner JOIN [dbo].[Split](@_Lots, ',') As S on proc_lots.code collate database_default = S.Item

BEGIN

Create Table tbl3 (code nvarchar(20), [name] nvarchar(100), [site] nvarchar(10), PackNumber nvarchar(20), [Weight] decimal(18,3), QTY int,
...
LEFT JOIN [FD_Innova].[dbo].proc_prunits AS pru with (nolock) on pru.prunit = pp.inventory
...
LEFT JOIN [FD_Innova].[dbo].proc_materialtypes AS pm2 with (nolock) on pm2.materialtype = pm.materialtype
left join [FD_Innova].[dbo].proc_pos as pos with (nolock) on pos.po = PP.po
left join [FD_Innova].[dbo].[proc_rmareas] as rma with (nolock) on rma.rmarea = PP.rmarea -- added by kh 16/11/18 FDr Into BH stamp
...
AND sp.[Sales Type] = '9' 
AND CASE WHEN sp.[Site Code] = '' THEN 'FD' ELSE SP.[Site Code] END = 'FD'
...
from [FD_Innova].[dbo].proc_items as it (nolock)
LEFT JOIN [FD_Innova].[dbo].proc_prunits AS pru with (nolock) on pru.prunit = it.inventory
LEFT JOIN [FD_Innova].[dbo].proc_lots AS pl with (nolock) on pl.lot = it.lot
LEFT JOIN [FD_Innova].[dbo].proc_materials AS pm with (nolock) on pm.material = it.material
LEFT JOIN [FD_Innova].[dbo].proc_materialtypes AS pm2 with (nolock) on pm2.materialtype = pm.materialtype
LEFT JOIN [FFGSQL02].[FFG-Production].[dbo].[FFG LIVE$Sales Price] AS SP WITH (NOLOCK) ON SP.[Item No_] COLLATE DATABASE_DEFAULT = pm.code 
AND sp.[Sales Type] = '9' 
AND CASE WHEN sp.[Site Code] = '' THEN 'FD' ELSE SP.[Site Code] END = 'FD'
