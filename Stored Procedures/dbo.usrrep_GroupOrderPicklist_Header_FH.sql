...
-- =============================================
Create PROCEDURE [dbo].[usrrep_GroupOrderPicklist_Header_FH]
...
SET @TotalKGQTY = (Select SUM(CAST(sl.[Original Ordered Qty_] as decimal(18,2))) as TotalKGQTY
			 ...
			 left join [FH_innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
...
Case when substring(pol.description6,1,1) = 0 THEN 'All'
	...
	from [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Header] as sh 
		...
		left join [FH_innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
		left join [FH_innova].[dbo].proc_materials as pm with (nolock) on  sl.No_ COLLATE DATABASE_DEFAULT = pm.extcode 
		left join [FH_innova].[dbo].proc_orderl as pol with (nolock) on po.[order] = pol.[order] and pol.material = pm.material
		left join [FH_innova].[dbo].base_companies as bc with (nolock) on bc.company = po.customer
