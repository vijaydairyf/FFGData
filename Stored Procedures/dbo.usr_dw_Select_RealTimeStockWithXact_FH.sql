...
CREATE PROCEDURE  [dbo].[usr_dw_Select_RealTimeStockWithXact_FH]
...
BEGIN
	...
	select 
	id,3 as site,sscc,pp.batch,

	-- DBogues : 2016-12-12 : TT Attempt at fix
...
(select code from [FH_INNOVA].[dbo].vw_matswithNoXML where material = pp.Material) as ProductCode,Packaging,weight,
pp.gross,tare,pieces,regtime, site as OrgSite,INV.description2, pp.rtype, pp.invtime	
...
inner join [FH_INNOVA].[dbo].vw_matswithNoXML mat (nolock) on PP.material = mat.material
left join [FH_INNOVA].[dbo].base_devices bd (nolock) on  bd.device= isnull(pp.device,6)

left join [FH_INNOVA].[dbo].[vw_LotswithNoXML] AS L With (nolock) on PP.lot = L.lot
