...
-- =============================================
CREATE PROCEDURE [dbo].[usrrep_PalletvsOrderline_Check_FC]
...
left join [FM_Innova].[dbo].proc_prunits as inv with (nolock) on inv.prunit = pp.inventory
...
left join [FM_Innova].[dbo].proc_orderl as pol with (nolock) on pol.[order] = po.[order]
...
from [FM_Innova].[dbo].proc_packs as pp
left join [FM_Innova].[dbo].proc_collections pc with (nolock) on pc.id= pp.pallet
left join [FM_Innova].[dbo].proc_lots as pl with (nolock) on pl.lot = pp.lot 
left join [FM_Innova].[dbo].proc_materials as pm with (nolock) on pm.material = pp.material
