...
Create PROCEDURE [dbo].[usrrep_Carcass_Dispatch_New_FG] 
...
From [FG_innova].[dbo].proc_items i
inner join [FG_innova].[dbo].proc_orders o on i.[order] = o.[order]
inner join [FG_innova].[dbo].proc_prunits r on i.inventory = r.prunit
inner join [FG_innova].[dbo].proc_materials m on m.material = i.material

Where r.prunit in (3) and cast(i.prday as date) between @from and @to
...
-- Select * from proc_prunits where name like '%despatch%'
