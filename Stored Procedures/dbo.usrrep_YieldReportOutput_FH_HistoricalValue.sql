...
-- =============================================
--exec [usrrep_YieldReportOutput] '09/17/2018','09/17/2018','151515111012',0,99999
CREATE PROCEDURE [dbo].[usrrep_YieldReportOutput_FH_HistoricalValue]
...
LEFT JOIN [FH_innova].[dbo].proc_prunits AS pru with (nolock) on pru.prunit = pp.inventory
...
LEFT JOIN [FH_innova].[dbo].proc_materialtypes AS pm2 with (nolock) on pm2.materialtype = pm.materialtype
left join [FH_innova].[dbo].proc_pos as pos with (nolock) on pos.po = PP.po
left join [FH_innova].[dbo].[proc_rmareas] as rma with (nolock) on rma.rmarea = PP.rmarea -- added by kh 16/11/18 FHr Into BH stamp
...
AND sp.[Sales Type] = '9' 
AND CASE WHEN sp.[Site Code] = '' THEN 'FH' ELSE SP.[Site Code] END = 'FH'
...
from [FH_Innova].[dbo].proc_items as it (nolock)
LEFT JOIN [FH_Innova].[dbo].proc_prunits AS pru with (nolock) on pru.prunit = it.inventory
LEFT JOIN [FH_Innova].[dbo].proc_lots AS pl with (nolock) on pl.lot = it.lot
LEFT JOIN [FH_Innova].[dbo].proc_materials AS pm with (nolock) on pm.material = it.material
LEFT JOIN [FH_Innova].[dbo].proc_materialtypes AS pm2 with (nolock) on pm2.materialtype = pm.materialtype
LEFT JOIN [FFGSQL02].[FFG-Production].[dbo].[FFG LIVE$Sales Price] AS SP WITH (NOLOCK) ON SP.[Item No_] COLLATE DATABASE_DEFAULT = pm.code 
AND sp.[Sales Type] = '9' 
AND CASE WHEN sp.[Site Code] = '' THEN 'FH' ELSE SP.[Site Code] END = 'FH'
