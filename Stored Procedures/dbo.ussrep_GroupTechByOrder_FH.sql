...
-- =============================================
CREATE PROCEDURE [dbo].[ussrep_GroupTechByOrder_FH]
...
-- exec [dbo].[GroupTechByOrder] '236435'
	...
	left join [FH_innova].[dbo].proc_countries as pc with (nolock) on pc.country = pl.brcountry
	left join [FH_innova].[dbo].proc_countries as pc1 with (nolock) on pc1.country = pl.ricountry
	left join [FH_innova].[dbo].base_companies as bc with (nolock) on bc.company = pl.slhouse
	left join [FH_innova].[dbo].base_companies as bc1 with (nolock) on bc1.company = pl.customer
	left join [FH_innova].[dbo].base_companies as bc2 with (nolock) on bc2.company = pl.processor
	left join [FH_innova].[dbo].proc_materialtypes as pmt with (nolock) on pmt.materialtype = pm.materialtype
	left join [FH_innova].[dbo].proc_orders as po with (nolock) on po.[order] = pp.[order]
	left join [FH_innova].[dbo].proc_collections as pal with (nolock) on pal.pallet = pp.pallet
