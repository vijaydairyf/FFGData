...
-- Description:	<FD_Innova into PackItem Table for reporting>
-- =============================================

--exec [dbo].[FD_Innova_ETL_Items]
CREATE PROCEDURE [dbo].[FD_Innova_ETL_Items] 
...
where LRP.TableName = 'tblItems' and [Site]='FD'
					   


SELECT	'Foyle Donegal' as [StockSite], PItems.number as ItemNumber, SUBSTRING(Pitems.extcode,1,1) as OriginalSite, ISNULL(pitems.extcode ,'')as ExternalCode, PL.code as LotCode, PL.[name] as LotName, IsNull(PItems.prday,'1753-01-01') as ProductionDate,
...
into #tmp2
FROM [FD_Innova].[dbo].[proc_items] AS PItems (nolock)

left join [FD_Innova].[dbo].[proc_lots] as PL with (nolock) on PL.lot = PItems.lot
left join [FD_Innova].[dbo].[base_devices] as BD with (nolock) on BD.device = PItems.device
left join [FD_Innova].[dbo].[proc_prunits] as PR with (nolock) on  PItems.inventory=PR.prunit
left join [FD_Innova].[dbo].[proc_invlocations] as INVLOC with (nolock) on INVLOC.id = PItems.invlocation
left join [FD_Innova].[dbo].[proc_materials] as MAT with (nolock) on MAT.material = PItems.material
left join [FD_Innova].[dbo].[proc_materialtypes] as MATTYPE with (nolock) on MATTYPE.materialtype = MAT.materialtype
left join [FD_Innova].[dbo].[base_employees] as BE with (nolock) on BE.employee = PItems.employee
left join [FD_Innova].[dbo].[proc_matxacts] as PMX with (nolock) on PMX.item = PItems.id
left join [FD_Innova].[dbo].[proc_lots] as PL2 with (nolock) on PL2.lot = PMX.tolot
left join [FD_Innova].[dbo].[proc_xactpaths] as XP with (nolock) on XP.xactpath = PMX.xactpath
...
using #tmp AS tmp
on pit.ItemNumber = tmp.ItemNumber and pit.itemID = tmp.itemID and pit.[StockSite] = 'Foyle Donegal'
...
Set TimeIndicator =( Select TOP 1 ItemRegtime FROM [FFGData].[dbo].[tblItems] where StockSite = 'Foyle Donegal' order by ItemRegtime desc)
From [FFGData].[dbo].[tblLRP] Where TableName='tblItems' and [Site] = 'FD'
