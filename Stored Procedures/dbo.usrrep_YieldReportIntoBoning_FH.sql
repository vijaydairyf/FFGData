...
-- =============================================
--exec [usrrep_YieldReportIntoBoning_FH] '01/30/2019','01/30/2019','141616101012',0,99999
CREATE PROCEDURE [dbo].[usrrep_YieldReportIntoBoning_FH]
...
from	[FH_innova].[dbo].proc_matxacts AS pmx
		LEFT Join [FH_innova].[dbo].proc_materials AS pm with (nolock) on pm.material = pmx.material
		LEFT Join [FH_innova].[dbo].proc_xactpaths AS xp with (nolock) on xp.xactpath = pmx.xactpath
		LEFT JOin [FH_innova].[dbo].proc_lots AS pl with (nolock) on pl.lot = pmx.tolot
...
from	[FH_innova].[dbo].proc_matxacts AS pmx
		...
		LEFT JOin [FH_innova].[dbo].proc_lots AS pl with (nolock) on pl.lot = pmx.tolot
		LEFT Join [FH_innova].[dbo].proc_xactpaths AS xp with (nolock) on xp.xactpath = pmx.xactpath
		LEFT JOIN 	[FH_innova].[dbo].proc_items as it with (nolock) on it.id = pmx.item
		LEFT JOIN [FH_innova].[dbo].proc_prunits AS pr WITH (NOLOCK) ON pr.prunit = ISNULL(it.inventory,0)
		Inner JOIN [dbo].[Split](@_Lots, ',') As S on pl.code = S.Item
...
from	[FH_innova].[dbo].proc_matxacts AS pmx
		...
		LEFT Join [FH_innova].[dbo].proc_xactpaths AS xp with (nolock) on xp.xactpath = pmx.xactpath
		LEFT JOIN 	[FH_innova].[dbo].proc_items as it with (nolock) on it.id = pmx.item
		LEFT JOIN [FH_innova].[dbo].proc_prunits AS pr WITH (NOLOCK) ON pr.prunit = it.inventory
		Inner JOIN [dbo].[Split](@_Lots, ',') As S on pl.code = S.Item
