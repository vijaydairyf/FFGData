...
-- =============================================
Create PROCEDURE [dbo].[ussrep_GroupTechByOrder_FMM]
...
-- exec [dbo].[GroupTechByOrder] '236435'
	...
	left join [FMM_innova].[dbo].proc_countries as pc with (nolock) on pc.country = pl.brcountry
	left join [FMM_innova].[dbo].proc_countries as pc1 with (nolock) on pc1.country = pl.ricountry
	left join [FMM_innova].[dbo].base_companies as bc with (nolock) on bc.company = pl.slhouse
	left join [FMM_innova].[dbo].base_companies as bc1 with (nolock) on bc1.company = pl.customer
	left join [FMM_innova].[dbo].base_companies as bc2 with (nolock) on bc2.company = pl.processor
	left join [FMM_innova].[dbo].proc_materialtypes as pmt with (nolock) on pmt.materialtype = pm.materialtype
	left join [FMM_innova].[dbo].proc_orders as po with (nolock) on po.[order] = pp.[order]
	left join [FMM_innova].[dbo].proc_collections as pal with (nolock) on pal.pallet = pp.pallet
