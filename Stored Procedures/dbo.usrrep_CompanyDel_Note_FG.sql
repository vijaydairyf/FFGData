...
Create PROCEDURE [dbo].[usrrep_CompanyDel_Note_FG]
...
left join [FG_Innova].[dbo].proc_collections pc with(nolock) on pp.pallet = pc.id
inner join [FG_Innova].[dbo].proc_lots l on pp.lot = l.lot
inner join [FG_Innova].[dbo].proc_orders po with(nolock) on pp.[order] = po.[order]
inner join [FG_Innova].[dbo].base_companies b with(nolock) on b.company = po.customer
