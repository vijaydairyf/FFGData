...
--EXEC [ussrep_Intercompany_ITEMS_FH] '2017-10-28','2017-11-06'
Create PROCEDURE [dbo].[ussrep_Intercompany_ITEMS_FH]
@StartDate datetime, 
@EndDate datetime
AS
BEGIN
...
-- sending site
  SELECT DISTINCT				 a.[name] as [Order No],m.[code] as [Product Code],m.[name] as [Product],b.extcode,
	...
	into #tmp
  FROM							[FH_innova].[dbo].proc_orders a

  INNER JOIN 	[FH_innova].[dbo].proc_items b on a.[order] = b.[order]
  INNER JOIN 	[FH_innova].[dbo].base_companies bc on a.customer = bc.company
  INNER JOIN 	[FH_innova].[dbo].proc_materials m on b.material = m.material
  INNER JOIN 	[FH_innova].[dbo].proc_prunits pr with(nolock) on b.inventory = pr.prunit

  WHERE							m.dimension1 in('70','10') and m.dimension2 = '14' and bc.[name] IN ('Foyle Ingredients','Foyle Gloucester','Foyle Campsie','Foyle Omagh','Foyle Donegal','Foyle Melton Mowbray','Donegal Meat Processors')  and cast(a.begtime as date) BETWEEN @StartDate and @EndDate
