...
--exec [dbo].[usrrep_GroupOrderPicklist_Header] 'FGL', '09/27/17','09/27/17','WMB','FFGSO254735'
Create PROCEDURE [dbo].[usrrep_GroupOrderPicklist_Header_FGL]
...
SET @TotalKGQTY = (Select SUM(CAST(sl.[Original Ordered Qty_] as decimal(18,2))) as TotalKGQTY
			 ...
			 left join [FG_innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
...
Case when substring(pol.description6,1,1) = 0 THEN 'All'
	...
	from [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Header] as sh 
		...
		left join [FG_innova].[dbo].proc_orders as po with (nolock) on po.[name]COLLATE DATABASE_DEFAULT =  sh.[No_] 
		left join [FG_innova].[dbo].proc_materials as pm with (nolock) on  sl.No_ COLLATE DATABASE_DEFAULT = pm.extcode 
		left join [FG_innova].[dbo].proc_orderl as pol with (nolock) on po.[order] = pol.[order] and pol.material = pm.material
		left join [FG_innova].[dbo].base_companies as bc with (nolock) on bc.company = po.customer

	
	--from proc_orders as po
	--left join proc_orderl as pol with (nolock) on po.[order] = pol.[order]
	--left join proc_materials as pm with (nolock) on pm.material = pol.material
	--left join base_companies as bc with (nolock) on bc.company = po.customer
	--left join [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Header] as sh with (nolock) on sh.[No_] COLLATE DATABASE_DEFAULT = po.[name]
	--left join [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Line] as sl with (nolock) on sl.[Document No_] = sh.[No_] and sl.No_ COLLATE DATABASE_DEFAULT = pm.extcode
	
	--select top 100 * from [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Header] where [No_] = 'FFGSO256187' order by 1 desc
	--select * from [ffgsql01].[FFG-Production].[dbo].[FFG LIVE$Sales Line] where [document no_] = 'FFGSO256187'
	where 
