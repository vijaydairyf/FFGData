...
CREATE PROCEDURE [dbo].[Group_Fresh_StockSnapshot_Daily_Insert_FH]
...
INSERT INTO [dbo].[Group_Fresh_Snapshot_Daily]
   ...
   SELECT GETDATE() AS ImportDate, 'FH' AS SITEID, pm.code AS ProductCode, pm.description1 AS ProductName, mt.[name] AS MaterialType, con.name AS [condition], pr.name AS Inventory,
...
FROM [FFGSQL03].[FH_Innova].dbo.proc_packs AS pp
	 left JOIN [FFGSQL03].[FH_Innova].dbo.proc_materials AS pm WITH (NOLOCK) ON pm.material = pp.material	
	 left JOIN [FFGSQL03].[FH_Innova].dbo.proc_materialtypes AS mt WITH (NOLOCK) ON mt.materialtype = pm.materialtype
	 left JOIN [FFGSQL03].[FH_Innova].dbo.proc_prunits AS pr WITH (NOLOCK) ON pr.prunit = pp.inventory
	 LEFT JOIN [FFGSQL03].[FH_Innova].dbo.proc_conditions AS con WITH (NOLOCK) ON pm.condition = con.condition	
