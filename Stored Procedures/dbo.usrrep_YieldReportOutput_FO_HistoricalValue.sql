...
-- =============================================
--exec [usrrep_YieldReportOutput_FO_test] '04/16/2019','04/16/2019','131313101012',0,99999
CREATE PROCEDURE [dbo].[usrrep_YieldReportOutput_FO_HistoricalValue]
...
from [FO_Innova].[dbo].Proc_Lots
Inner JOIN [dbo].[Split](@_Lots, ',') As S on proc_lots.code = S.Item

BEGIN

Create Table tbl1 (code nvarchar(20), [name] nvarchar(100), [site] nvarchar(10), PackNumber nvarchar(20), [Weight] decimal(18,3), QTY int,
...
LEFT JOIN [FO_Innova].[dbo].proc_prunits AS pru with (nolock) on pru.prunit = pp.inventory
...
LEFT JOIN [FO_Innova].[dbo].proc_materialtypes AS pm2 with (nolock) on pm2.materialtype = pm.materialtype
left join [FO_Innova].[dbo].proc_pos as pos with (nolock) on pos.po = PP.po
left join [FO_Innova].[dbo].[proc_rmareas] as rma with (nolock) on rma.rmarea = PP.rmarea -- added by kh 16/11/18 for Into BH stamp
...
AND sp.[Sales Type] = '9' 
AND CASE WHEN sp.[Site Code] = '' THEN 'FO' ELSE SP.[Site Code] END = 'FO'
...
from [FO_Innova].[dbo].proc_items as it (nolock)
LEFT JOIN [FO_Innova].[dbo].proc_prunits AS pru with (nolock) on pru.prunit = it.inventory
LEFT JOIN [FO_Innova].[dbo].proc_lots AS pl with (nolock) on pl.lot = it.lot
LEFT JOIN [FO_Innova].[dbo].proc_materials AS pm with (nolock) on pm.material = it.material
LEFT JOIN [FO_Innova].[dbo].proc_materialtypes AS pm2 with (nolock) on pm2.materialtype = pm.materialtype
LEFT JOIN [FFGSQL02].[FFG-Production].[dbo].[FFG LIVE$Sales Price] AS SP WITH (NOLOCK) ON SP.[Item No_] COLLATE DATABASE_DEFAULT = pm.code 
AND sp.[Sales Type] = '9' 
AND CASE WHEN sp.[Site Code] = '' THEN 'FO' ELSE SP.[Site Code] END = 'FO'
