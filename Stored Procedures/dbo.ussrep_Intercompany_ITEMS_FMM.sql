...
--exec [ussrep_Intercompany_ITEMS_FMM] '2017-11-01','2017-11-06'
Create PROCEDURE [dbo].[ussrep_Intercompany_ITEMS_FMM]
...
-- sending site
  SELECT DISTINCT				 a.[name] as [Order No],m.[code] as [Product Code],m.[name] as [Product],b.extcode, 
								...
								b.nominal as [WGT],b.[value] as [Value],
								 bc.[name] as [Rec Site], cast(a.dispatchtime as date) as [Dispatch Time], p.[name] as [Inventory], case when a.orderstatus = 1 then 'Closed'
	...
	into #tmp
  FROM							[FMM_innova].[dbo].proc_orders a

  INNER JOIN [FMM_innova].[dbo].proc_items b on a.[order] = b.[order]
  INNER JOIN [FMM_innova].[dbo].base_companies bc on a.customer = bc.company
  INNER JOIN [FMM_innova].[dbo].proc_materials m on b.material = m.material
  INNER JOIN [FMM_innova].[dbo].proc_prunits p with(nolock) on b.inventory = p.prunit

  WHERE		m.dimension1 in('70','10') and m.dimension2 = '14'
			and bc.[name] IN ('Foyle Ingredients','Foyle Gloucester','Foyle Campsie','Foyle Hilton','Foyle Donegal','Foyle Omagh','Donegal Meat Processors') 
				and cast(a.begtime as date) between @StartDate and @EndDate 
  GROUP BY						a.[name],m.code,a.[name],m.[name],bc.[name], a.dispatchtime,b.nominal,b.[value], b.extcode,p.[name], a.orderstatus
	...
	select extcode into #grouptmp from FO_Innova.dbo.proc_items where prday between @StartDate -5 and @EndDate  --or prday >= GETDATE()-14
