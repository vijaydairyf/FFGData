...
-- =============================================
--exec [usrrep_YieldReportIntoBoning_FC] '02/13/2019','02/13/2019','141111101012,111111111011',0,99999
CREATE PROCEDURE [dbo].[usrrep_YieldReportIntoBoning_FC]
...
from	[FM_innova].[dbo].proc_matxacts AS pmx
		...
		LEFT JOIN [FM_innova].[dbo].proc_prunits AS pr WITH (NOLOCK) ON pr.prunit = ISNULL(it.inventory,'0')
		Inner JOIN [dbo].[Split](@_Lots, ',') As S on pl.code collate DATABASE_DEFAULT = S.Item
...
it.inventory, 
pmx.prday, pmx.[weight] , pmx.nregs
...
from	[FM_innova].[dbo].proc_matxacts AS pmx
		...
		LEFT Join [FM_innova].[dbo].proc_xactpaths AS xp with (nolock) on xp.xactpath = pmx.xactpath
		LEFT JOIN 	[FM_innova].[dbo].proc_items as it with (nolock) on it.id = pmx.item
		LEFT JOIN [FM_innova].[dbo].proc_prunits AS pr WITH (NOLOCK) ON pr.prunit = it.inventory
		Inner JOIN [dbo].[Split](@_Lots, ',') As S on pl.code collate DATABASE_DEFAULT = S.Item
