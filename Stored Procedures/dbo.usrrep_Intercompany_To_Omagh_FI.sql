...
Create PROCEDURE [dbo].[usrrep_Intercompany_To_Omagh_FI]
@StartDate date,
@EndDate date
AS
BEGIN
...
FROM									[FI_Innova].[dbo].proc_collections pc

	INNER JOIN [FI_Innova].[dbo].proc_packs pp with (nolock) on pc.id = pp.pallet
	INNER JOIN [FI_Innova].[dbo].proc_materials pm with (nolock) on pp.material = pm.material
	LEFT JOIN [FI_Innova].[dbo].proc_orders po with (nolock) on pc.[order] = po.[order]
	LEFT JOIN [FI_Innova].[dbo].base_companies bc with(nolock) on po.customer = bc.company
	left join [FI_Innova].[dbo].proc_prunits pru with(nolock) on pp.inventory = pru.prunit 
