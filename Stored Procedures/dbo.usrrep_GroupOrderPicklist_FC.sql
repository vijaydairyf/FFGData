...
-- =============================================
Create PROCEDURE [dbo].[usrrep_GroupOrderPicklist_FC]
...
Create Table table3(  OrderNo nvarchar(max),ProductCode nvarchar(10), ProductDescription nvarchar(max), PackDate datetime, UseBy Datetime, DNOB Datetime, 
...
Select @maxorderlines  =  COUNT(pol.id)
					from [FM_Innova].[dbo].proc_orderl pol
					left join [FM_Innova].[dbo].proc_orders as po with (nolock) on po.[order] = pol.[order]
...
Select TOP 1 @Sequence =  CAST(pl.description7 as int) from [FM_Innova].[dbo].proc_orderl as pl 
						  left join [FM_Innova].[dbo].proc_orders as po with (nolock) on po.[order] = pl.[order] 
...
Declare @Order nvarchar(max),@ProductCode nvarchar (15), @ProductDescription nvarchar (max), @PrdayMax datetime, @PrdayMin datetime, @UsebyMax datetime, @UsebyMin datetime, @DNOBMax datetime, @DNOBMin datetime,
	...
	Select 	@Order = po.[name], @ProductCode = pm.code, @ProductDescription = pm.description1 , @PrdayMax = pol.prdaymax, @PrdayMin = pol.prdaymin, @UsebyMax =	pol.expire1max, @UsebyMin =	pol.expire1min,
			...
			from [FM_Innova].[dbo].proc_orderl as pol (nolock)
			left join [FM_Innova].[dbo].proc_orders as po with (nolock) on po.[order] = pol.[order]
			left join [FM_Innova].[dbo].proc_materials as pm with (nolock) on pm.material = pol.material
...
--------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	left join [FM_Innova].[dbo].proc_materials as pm with (nolock) on pp.material = pm.material
	...
	left join [FM_Innova].[dbo].proc_invlocations as invl with (nolock) on invl.id = pp.invlocation
