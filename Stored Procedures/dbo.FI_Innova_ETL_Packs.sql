...
-- Description:	<FI_Innova into PackItem Table for reporting>
-- =============================================

--exec [dbo].[FI_Innova_ETL_Packs] 

CREATE PROCEDURE [dbo].[FI_Innova_ETL_Packs] 
...
where LRP.TableName = 'tblPack' and [Site]='FI'
...
create table #TMP2(
					...
					[CustomerBrand] nvarchar(100), [FrozenGrouping] nvarchar(30), [MaterialType] nvarchar(30), [ProductSubGrouping] nvarchar(30) ,[FreshFrozen] nvarchar(30),
					[Weight] decimal (18,2), [Pieces] int, [Xacttime] datetime, 
					[ProductionLocation] nvarchar(50), [InventoryTime] datetime, [BatchNumber] nvarchar(50)
...
SELECT	'Foyle Ingredients' as [StockSite], pp.number as PackNumber, sites.[name] as OrginalSite, pp.sscc as SSCC, PL.code AS LotCode, PL.[name] as LotName, IsNull(PC.number,'') as PalletNo, IsNull(PP.prday,'1753-01-01') as ProductionDate, 
...
left join [FI_Innova].[dbo].[proc_lots] as PL with (nolock) on PL.lot = PP.lot
left join [FI_Innova].[dbo].[base_devices] as BD with (nolock) on BD.device = PP.device
left join [FI_Innova].[dbo].[proc_prunits] as PR with (nolock) on  PP.inventory=PR.prunit
left join [FI_Innova].[dbo].[proc_invlocations] as INVLOC with (nolock) on INVLOC.id = pp.invlocation
left join [FI_Innova].[dbo].[proc_materials] as MAT with (nolock) on MAT.material = PP.material
left join [FI_Innova].[dbo].[proc_conditions] as COND with (nolock) on COND.condition = MAT.condition
left join [FI_Innova].[dbo].[proc_conservations] as CONS with (nolock) on CONS.conservation = MAT.conservation
left join [FI_Innova].[dbo].[proc_materialtypes] as MATTYPE with (nolock) on MATTYPE.materialtype = MAT.materialtype
left join [FI_Innova].[dbo].[base_sites] as Sites with (nolock) on  Sites.[site] = pp.[site] -- added by KH -- Changed From Dimension1 to Site by MMG 06/04/2017
left join [FI_Innova].[dbo].[proc_collections] as PC with (nolock) on PC.id = PP.pallet
left join [FI_Innova].[dbo].[base_companies] co WITH (NOLOCK) on PL.processor = co.company
left join [FO_Innova].[dbo].[proc_orders] po with (nolock) on po.[order] = PP.[order]
left join [FO_Innova].[dbo].[base_companies] co2 WITH (NOLOCK) on co2.company = po.customer
...
using #tmp AS tmp
on pit.sscc = tmp.sscc and pit.[StockSite] = 'Foyle Ingredients'
...
Set TimeIndicator =( Select TOP 1 Xacttime FROM [FFGData].[dbo].[tblPack] where StockSite = 'Foyle Ingredients' order by Xacttime desc)
From [FFGData].[dbo].[tblLRP] Where TableName='tblPack' and [Site] = 'FI'
