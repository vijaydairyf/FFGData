...
-- =============================================
CREATE PROCEDURE [dbo].[usrrep_PalletvsOrderline_Check_FMM]
...
left join [FMM_innova].[dbo].proc_prunits as inv with (nolock) on inv.prunit = pp.inventory
...
left join [FMM_innova].[dbo].proc_orderl as pol with (nolock) on pol.[order] = po.[order]
...
from [FMM_innova].[dbo].proc_packs as pp
left join [FMM_innova].[dbo].proc_collections pc with (nolock) on pc.id= pp.pallet
left join [FMM_innova].[dbo].proc_lots as pl with (nolock) on pl.lot = pp.lot 
left join [FMM_innova].[dbo].proc_materials as pm with (nolock) on pm.material = pp.material
