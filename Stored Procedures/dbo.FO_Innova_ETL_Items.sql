...
--exec [dbo].[FO_Innova_ETL_Items] 

CREATE PROCEDURE [dbo].[FO_Innova_ETL_Items] 
...
where LRP.TableName = 'tblItems' and [Site]='FO'


SELECT	'Foyle Omagh' as [StockSite], PItems.number as ItemNumber, SUBSTRING(Pitems.extcode,1,1) as OriginalSite, ISNULL(pitems.extcode ,'')as ExternalCode, PL.code as LotCode, PL.[name] as LotName, IsNull(PItems.prday,'1753-01-01') as ProductionDate,
...
into #tmp2
FROM [FO_Innova].[dbo].[proc_items] AS PItems (nolock)

left join [FO_Innova].[dbo].[proc_lots] as PL with (nolock) on PL.lot = PItems.lot
left join [FO_Innova].[dbo].[base_devices] as BD with (nolock) on BD.device = PItems.device
left join [FO_Innova].[dbo].[proc_prunits] as PR with (nolock) on  PItems.inventory=PR.prunit
left join [FO_Innova].[dbo].[proc_invlocations] as INVLOC with (nolock) on INVLOC.id = PItems.invlocation
left join [FO_Innova].[dbo].[proc_materials] as MAT with (nolock) on MAT.material = PItems.material
left join [FO_Innova].[dbo].[proc_materialtypes] as MATTYPE with (nolock) on MATTYPE.materialtype = MAT.materialtype
left join [FO_Innova].[dbo].[base_employees] as BE with (nolock) on BE.employee = PItems.employee
left join [FO_Innova].[dbo].[proc_matxacts] as PMX with (nolock) on PMX.item = PItems.id
left join [FO_Innova].[dbo].[proc_lots] as PL2 with (nolock) on PL2.lot = PMX.tolot
left join [FO_Innova].[dbo].[proc_xactpaths] as XP with (nolock) on XP.xactpath = PMX.xactpath
...
using #tmp AS tmp
on pit.ItemNumber = tmp.ItemNumber and pit.itemID = tmp.itemID and pit.[StockSite] = 'Foyle Omagh'
...
Set TimeIndicator =( Select TOP 1 ItemRegtime FROM [FFGData].[dbo].[tblItems] where stocksite = 'Foyle Omagh' order by ItemRegtime desc)
From [FFGData].[dbo].[tblLRP] Where TableName='tblItems' and [Site] = 'FO'
