...
CREATE PROCEDURE [dbo].[USREP_HR_Dispatch_Summary_Report_FI] 
...
--set @NAVOrder = ('%' + cast(@OrderNo as nvarchar(15))  + '%')
		select 'FI' as site,count(pk.id) as QTY  ,pm.description1 collate SQL_Latin1_General_CP1_CI_AS as [Product Name] , 
	...
	From [FI_INNOVA].dbo.[proc_packs] pk
	left join [FI_INNOVA].[dbo].[vw_OrderswithNoXML] po (nolock) on pk.[order]= po.[order]
	left join [FI_INNOVA].[dbo].[vw_matswithNoXML] pm (nolock) on pk.material = pm.material
	left join [FI_INNOVA].[dbo].[vw_LotswithNoXML] pl (nolock) on pk.lot = pl.lot
	left join [FI_INNOVA].dbo.[proc_collections] pc (nolock) on pk.pallet = pc.id
