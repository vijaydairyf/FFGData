...
-- =============================================
Create PROCEDURE [dbo].[usrrep_GroupOrderPicklist_FI]

--exec [dbo].[usrrep_GroupOrderPicklist] 'FFGSO306490'
--exec [dbo].[usrrep_GroupOrderPicklist] 'FFGSO253285,FFGSO253284'
...
Create Table table7(  OrderNo nvarchar(max),ProductCode nvarchar(max), ProductDescription nvarchar(max), PackDate datetime, UseBy Datetime, DNOB Datetime, 
					KillDate datetime, Lot nvarchar(max), Inventory nvarchar(max), InvLocation nvarchar(max), Pallet nvarchar(max),
					QTY bigint)
...
Select @maxorderlines  =  COUNT(pol.id)
					from [FI_Innova].[dbo].proc_orderl pol
					left join [FI_Innova].[dbo].proc_orders as po with (nolock) on po.[order] = pol.[order]
...
Select TOP 1 @Sequence =  CAST(pl.description7 as int) from [FI_Innova].[dbo].proc_orderl as pl 
						  left join [FI_Innova].[dbo].proc_orders as po with (nolock) on po.[order] = pl.[order] 
...
Declare @Order nvarchar(max),@ProductCode nvarchar (15), @ProductDescription nvarchar (max), @PrdayMax datetime, @PrdayMin datetime, @UsebyMax datetime, @UsebyMin datetime, @DNOBMax datetime, @DNOBMin datetime,
	...
	Select 	@Order = po.[name], @ProductCode = pm.code, @ProductDescription = pm.description1 , @PrdayMax = pol.prdaymax, @PrdayMin = pol.prdaymin, @UsebyMax =	pol.expire1max, @UsebyMin =	pol.expire1min,
			@DNOBMax = pol.expire3max, @DNOBMin = pol.expire3min, @ValLots = pol.ValidLots, @KillDateMin = pol.expire2min , @KillDateMax = pol.expire2max, @constraint = substring(pol.description6,1,1)
			
			from [FI_Innova].[dbo].proc_orderl as pol (nolock)
			left join [FI_Innova].[dbo].proc_orders as po with (nolock) on po.[order] = pol.[order]
			left join [FI_Innova].[dbo].proc_materials as pm with (nolock) on pm.material = pol.material
...
--------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		INSERT into table7
		   ...
		   Select @order as [OrderNo], pm.code as ProductCode,pm.description1 as ProductDescription, pp.prday as PackDate, pp.expire1 as UseBy, 
			...
			substring(pl.code,1,12) as Lot, pr.[name] as Inventory, 
	...
	left join [FI_Innova].[dbo].proc_materials as pm with (nolock) on pp.material = pm.material
	...
	left join [FI_Innova].[dbo].proc_invlocations as invl with (nolock) on invl.id = pp.invlocation
