...
-- =============================================
CREATE PROCEDURE [dbo].[usrrep_PalletvsOrderline_Check_FO]
	-- Add the parameters for the stored procedure here
--exec [dbo].[usrrep_PalletvsOrderline_Check_FO] '380549','364101'
...
left join [FO_Innova].[dbo].proc_prunits as inv with (nolock) on inv.prunit = pp.inventory
...
left join [FO_Innova].[dbo].proc_orderl as pol with (nolock) on pol.[order] = po.[order]
...
from [FO_Innova].[dbo].proc_packs as pp
left join [FO_Innova].[dbo].proc_collections pc with (nolock) on pc.id= pp.pallet
left join [FO_Innova].[dbo].proc_lots as pl with (nolock) on pl.lot = pp.lot 
left join [FO_Innova].[dbo].proc_materials as pm with (nolock) on pm.material = pp.material
